...
class _NotificationsModalState extends State<NotificationsModal> {
    ...
    return FractionallySizedBox(
      ...
      child: Column(
        children: [
          ...
          Divider(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.24), height: 1),
          Expanded(
            child: _buildBody(),
          ),
        ],
  ...
  Widget _buildBody() {
    ...
    if (_notifications!.isEmpty) {
      return Center(child: Text('No notifications.', style: TextStyle(color: Theme.of(context).colorScheme.onSurface.withOpacity(0.54))));
    }
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          leading: CircleAvatar(
            backgroundColor: item.read ? Theme.of(context).colorScheme.surface : UsalamaTheme.primaryRed.withOpacity(0.2),
            child: Icon(
              item.read ? Icons.notifications_none : Icons.notifications_active,
              color: item.read ? Theme.of(context).colorScheme.onSurface.withOpacity(0.54) : UsalamaTheme.primaryRed,
            ),
          ),
          title: Text(
            item.message,
